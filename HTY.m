...
for k = 2:MAX_ITER
    ...
    % X-update
    [Q,P] = eig( C - lambda- beta*X );
    es = diag(P);
    xi = (-es + sqrt(es.^2 + 4*beta))./(2*beta);
    X = Q*diag(xi)*Q';
    
    %lambda_mid-update
    lambda_mid= lambda- beta*(X-S+L);
    
    % S-update with relaxation
    tempp=lambda_mid/(u*beta);
    S_tem = S_old-tempp;
    v_tem = v/(u*beta);
    S = shrinkage(S_tem, v_tem);
    
    %L-update
    L_tem = L_old+tempp-EY/beta;
    [W,T] = eig(L_tem);
    L = W*diag(max(diag(T),0))*W';
    
    % lambda_k+1-update
    lambda = lambda - H*(X-S+L);
